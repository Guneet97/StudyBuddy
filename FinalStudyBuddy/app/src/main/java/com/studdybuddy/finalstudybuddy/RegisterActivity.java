        ...
        registerBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!TextUtils.isEmpty(email) && !TextUtils.isEmpty(password)){
                    SBauthentication.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            String user_id = SBauthentication.getCurrentUser().getUid();
                            DatabaseReference current_user_db = StudyDatabase.child(user_id);
